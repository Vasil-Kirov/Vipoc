...
#define swapf(a, b)                       \
	{                                     \
		float reserved_variable_name = a; \
		a = b;                            \
		b = reserved_variable_name;       \
	}
#define swapv3(a, b)                   \
	{                                  \
		v3 reserved_variable_name = a; \
		a = b;                         \
		b = reserved_variable_name;    \
	}
...
#define CHECK_GL_ERROR()                                              \
	{                                                                 \
		...
		if (error != 0)                                               \
		{                                                             \
			VP_ERROR("OPENGL ERROR at line %d: %d", __LINE__, error); \
		}                                                             \
	}
...
void pushed_objects_to_verts();

void draw_ui_targets();

#define SECONDS_SINCE_START() ((float)platform_get_ns_since_start() / 10000000.0f)
...
void set_shader_uniform_vec3(char *str, v3 vector)
{
...
void set_shader_uniform_f(char *str, float to_set)
{
	GLint location = glGetUniformLocation(renderer.shader_program, str);
	if (glGetError() != 0)
		CHECK_GL_ERROR();
...
load_bmp_file(char *path)
{
	...
	// Should be fine ?
	entire_file file = {};
	file.contents = vp_allocate_temp(MB(10));
	...
	if (file.size != 0)
	{
		...
		void *asset_memory = vp_allocate_asset(pixels_size);
		char *pixels_ptr = file.contents + header->BitmapOffset;
		memcpy(asset_memory, pixels_ptr, pixels_size);
		result.pixels = asset_memory;
...
int compare_targets(void const *a, void const *b)
{
	return (((ui_targets *)a)->target.layer_id - ((ui_targets *)b)->target.layer_id);
}

void sort_2d_target(vp_2d_render_target *targets, int size)
{
	START_DTIMER();
...
void vp_render_targets(vp_2d_render_target *targets, int size, v4 color)
{

	vp_mesh_identifier info = render_targets_to_gl_buffers(targets, size);
	set_uniforms_for_ui();
...
void vp_draw_rectangle(m2 position, v4 color, int layer_id)
{
...
v4 u32_to_v4(u32 value)
{
	v4 result;
	result.x = normalize_between((value >> 24) & 0xFF, 0, 255, -1, 1);
	result.y = normalize_between((value >> 16) & 0xFF, 0, 255, -1, 1);
	result.z = normalize_between((value >> 8) & 0xFF, 0, 255, -1, 1);
	result.w = normalize_between((value >> 0) & 0xFF, 0, 255, -1, 1);
	return result;
}

void vp_draw_text(char *text, float x, float y, u32 in_color, float scaler, int layer_id)
{

	START_DTIMER();

	// RGBA
	v4 color = u32_to_v4(in_color);
	...
	vp_render_targets(character_verts, last_character, color);
...
#define ToAndPastC(at, chr)    \
	at = strchr(at, (int)chr); \
	at++
...
vp_load_simple_obj(char *path)
{
	...
	obj_file.contents = vp_allocate_temp(platform_get_size_of_file(path));
	if (!platform_read_entire_file(path, &obj_file))
		VP_FATAL("MISSING FILE %s", path);
...
#define MAX_NORMS MAX_VERTS
	...
	v3 *mesh_verts = vp_allocate_temp(size);

	u32 last_vert = 0;
	u32 last_norm = 0;

	u32 *indexes = vp_allocate_temp(MAX_INDEXES * sizeof(u32));
	u32 last_index = 0;

	u32 *norm_indexes = vp_allocate_temp(MAX_NORM_INDEXES * sizeof(u32));
	u32 last_norm_index = 0;

	v3 *normals = vp_allocate_temp(MAX_NORMS * sizeof(f32));
	...
	mesh_vertex *verts = vp_allocate_temp(last_index * sizeof(mesh_vertex));
	...
	u32 *fin_indexes = vp_allocate_temp(last_index * sizeof(u32));
	...
	{
		VP_ERROR("Indexes in obj file exceed maximum");
		assert(false);
	}
	if (last_norm_index > MAX_NORM_INDEXES)
	{
		VP_ERROR("Norm indexes in obj file exceed maximum");
		assert(false);
	}
	if (last_norm > MAX_NORMS)
	{
		VP_ERROR("Norms in obj file exceed maximum");
		assert(false);
	}
	if (last_vert > MAX_VERTS)
	{
		VP_ERROR("Verts in obj file exceed maximum");
		assert(false);
	}
...
void vp_load_texture(char *path)
{
	vp_texture result = {};
	if (strstr(path, ".bmp") != vp_nullptr)
	{
...
void renderer_init()
{
	...
	vstd_strcat(vertex_shader_location, "engine\\src\\renderer\\gl_shaders\\shader.vert");
	vstd_strcat(fragment_shader_location, "engine\\src\\renderer\\gl_shaders\\shader.frag");

	entire_file vertex_code_file = {};
	vertex_code_file.contents = vertex_shader_source;

	entire_file fragment_code_file = {};
	fragment_code_file.contents = fragment_shader_source;
