...
internal void *raw_input_memory;
internal u64 perf_counter_at_start;
internal u64 perf_freq;
...
platform_init(vp_config game, platform_state *pstate)
{
	...
	if (RegisterClassA(&wnd))
	{
		...
		{
			raw_input_memory = vp_allocate_perm(MB(1));
			...
			LARGE_INTEGER large_perf_count;
			if (!QueryPerformanceCounter(&large_perf_count))
				VP_FATAL("Couldn't query performance counter: %d", GetLastError());

			perf_counter_at_start = large_perf_count.QuadPart;

			LARGE_INTEGER large_perf_freq;
			if (!QueryPerformanceFrequency(&large_perf_freq))
				VP_FATAL("Couldn't query performance frequency: %d", GetLastError());

			perf_freq = large_perf_freq.QuadPart;
...
void Win32LoadRawInput(HWND Window)
{
	...
	if (RegisterRawInputDevices(rids, 5, sizeof(RAWINPUTDEVICE)) == FALSE)
		Error("RegisterRawInputDevice failed!\n");
}

void platform_get_absolute_path(char *output)
{
...
u64 platform_get_ns_since_start()
{
	LARGE_INTEGER perf_counter;
	if (!QueryPerformanceCounter(&perf_counter))
		return 0;

	double seconds = ((double)(perf_counter.QuadPart - perf_counter_at_start) / (double)perf_freq);

	return (u64)(seconds * 10000000.0);
}
int64 platform_get_frequency()
{
...
int64 platform_get_perf_counter()
{
	LARGE_INTEGER result;
...
b32 platform_key_to_char(vp_keys key, char *out)
{
	WORD output;
...
void HandleRawInput(RAWINPUT *raw_input)
{
	...
	case RIM_TYPEKEYBOARD:
	{
		...
		if (key_callback != vp_nullptr)
			key_callback(raw_input->data.keyboard.VKey, is_down);
	}
	break;
	case RIM_TYPEMOUSE:
	{
		...
		case RI_MOUSE_LEFT_BUTTON_DOWN:
		{
			...
			if (button_callback)
				button_callback(VP_MOUSE_BUTTON_LEFT, true, x, y);
		}
		break;
		case RI_MOUSE_RIGHT_BUTTON_DOWN:
		{
			...
			if (button_callback)
				button_callback(VP_MOUSE_BUTTON_RIGHT, true, x, y);
		}
		break;
		...
		case RI_MOUSE_MIDDLE_BUTTON_DOWN:
		{
			...
			if (button_callback)
				button_callback(VP_MOUSE_BUTTON_MIDDLE, true, x, y);
		}
		break;
		...
		case RI_MOUSE_LEFT_BUTTON_UP:
		{
			...
			if (button_callback)
				button_callback(VP_MOUSE_BUTTON_MIDDLE, false, x, y);
		}
		break;
		...
		case RI_MOUSE_BUTTON_4_UP:
		{
			...
			if (button_callback)
				button_callback(VP_MOUSE_BUTTON_XBUTTON1, false, x, y);
		}
		break;
		case RI_MOUSE_BUTTON_5_UP:
		{
			...
			if (button_callback)
				button_callback(VP_MOUSE_BUTTON_XBUTTON2, false, x, y);
		}
		break;
		default:
		{
...
void platform_allocate_console()
{
	AllocConsole();
}

void platform_output_string(char *str, uint8 color)
{
...
void platform_free_sharable(platform_sharable sharable)
{
...
{
	if (CopyFileA(old_path, new_path, FALSE) == 0)
		return false;
	return true;
}
...
void platform_switch_fullscreen()
{
	...
	LPARAM lParam)
{
	...
	case WM_INPUT:
	{
		...
		if (GetRawInputData((HRAWINPUT)lParam, RID_INPUT, (PRAWINPUT)raw_input_memory, &DataSize, sizeof(RAWINPUTHEADER)) == -1)
		{
			Error("Failed getting raw input\n");
		}
		HandleRawInput(raw_input_memory);
	}
	break;
	case WM_MOUSEMOVE:
	{
...
{
	return (renderer_toggle_vsync(toggle));
}
#endif

#endif

/* Old windows opengl code, to be replaced with d3d */
#if defined(RENDERER_GL)
...
void LoadOpenGLExtensions()
{
	...
	DescribePixelFormat(dummy_dc, SuggestedPixelFormatIndex, sizeof(PIXELFORMATDESCRIPTOR),
						&pfd);
	if (!SetPixelFormat(dummy_dc, SuggestedPixelFormatIndex, &pfd))
		Error("Failed to set pixel format for the dummy context");
	HGLRC OpenGLRC = wglCreateContext(dummy_dc);
	if (!wglMakeCurrent(dummy_dc, OpenGLRC))
		Error("Failed to make current the dummy context");
