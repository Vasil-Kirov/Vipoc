...
i32 find_valid_index(u32 id, const char *name)
{
...
void vp_start_debug_timer(const char *name, u32 id)
{
#if defined(VIPOC_DEBUG)
	i32 index = find_valid_index(id, name);
	debug.timers[index].value = platform_get_ns_since_start(); // vp_debug_timer();
	debug.timers[index].id = id;
	debug.timers[index].name = name;
#endif
}

void vp_stop_debug_timer(u32 id)
{
#if defined(VIPOC_DEBUG)
	i32 index = find_valid_index(id, "");
	debug.draw_timers[index].value += platform_get_ns_since_start() /*vp_debug_timer()*/ - debug.timers[index].value;
...
u16 count_diagrams()
{
	...
	{
		if (debug.draw_timers[index].id != 0)
			counter++;
	}
	return counter;
}
...
void vp_draw_diagrams()
{
	...
	for (int index = 0; index < MAX_TIMERS; ++index)
	{
		...
		vstd_sprintf(text_to_draw, "%s %llu ns", debug.draw_timers[index].name, debug.draw_timers[index].value);
		...
		//		VP_WARN("%s %llu ms", debug.draw_timers[index].name, debug.draw_timers[index].value / 1000);
