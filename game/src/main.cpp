...
#define Allocate(Size, Storage) \
	Storage.End;                \
	Storage.End = (char *)Storage.End + Size
#define Free(Storage)                                                      \
	memset(Storage.Start, 0, (char *)Storage.End - (char *)Storage.Start); \
...
void SetConfigRes(int Width, int Height)
{
...
LoadSharable(char *PathToFolder)
{
	...
	if (platform_copy_file(DLLPath, TempDLLPath) == FALSE)
		VP_ERROR("Failed to copy sharable!");
	...
	if (Result.UpdateAndRender == vp_nullptr)
		VP_ERROR("Failed to get GameUpdateAndRender function!");
	return Result;
}

void CreateSnowParticles(uint64 *RandomSeed)
{
	// 16 : 9
	// 20 : 20
...
int main()
{
	...
	Config.config.name = "3D Rendering";
